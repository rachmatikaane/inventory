...
class TransaksiController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				transaksi.created_at LIKE ? OR 
				transaksi.updated_at LIKE ?
			)";
			$search_params = array(
				"%$text%","%$text%","%$text%"
			);
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("transaksi.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Transaksi";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		...
		return $this->render_view("transaksi/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("created_at","updated_at");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","created_at","updated_at");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'created_at' => 'required',
	...
	function delete($rec_id = null){
		...
		$db->where("transaksi.id", $arr_rec_id, "in");
